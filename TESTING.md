...
In CI, each OS runs the UI suite in two parallel jobs so that neither half outgrows its step limits: one job runs the preflight, main, and composer phases, and the other runs the profile phase. The main and profile phases each have a 45-minute step limit, which leaves room for slower hosted runners, and the composer phase a 30-minute step limit. On iPadOS 26, the `ipad-ui-tests` job has a 105-minute outer limit and the `ipad-profile-ui-tests` job a 65-minute one. On iPadOS 18, the `minimum-ios-18-ui-tests` job has a 105-minute outer limit. The `minimum-ios-18` job pairs the iOS 18 unit tests with the shorter profile phase, reuses the UI derived-data directory for that sequential unit step, and has a 75-minute outer limit. Every outer limit provides headroom for result upload even if each of its UI phases reaches its own limit.
